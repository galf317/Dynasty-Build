...
window.submitMultiplayerResult = async function(champion){
  const result = {
    ...
    playoffStage: finalPlayoffStage,
    traits: picks,
  ...
  if(!window.__LEADERBOARD_SUBMITTED){
    ...
    await firebase.database().ref("leaderboard").push({
      ...
      playoffStage: result.playoffStage,
...
function postseasonRank(result){
  if(result.champion || result.playoffStage==="champion") return 6;
  const ranks={
    super_bowl:5,
  ...
  return ranks[result.playoffStage] || 0;
}

function postseasonLabel(result){
  if(result.champion || result.playoffStage==="champion") return "Super Bowl Champion";
  const labels={
    super_bowl:"Super Bowl Appearance",
    conference_championship:"Conference Championship",
    divisional:"Divisional Round",
    wild_card:"Wild Card",
    missed_playoffs:"Missed Playoffs"
  };
  return labels[result.playoffStage] || "Legacy Entry";
}

function multiplayerScore(result){
  return (
    postseasonRank(result) * 1000000 +
    result.wins * 10000 -
    result.losses * 100 +
    result.overall
  );
...
function leaderboardSort(first,second){
  const stageDifference=postseasonRank(second)-postseasonRank(first);
...
async function loadLeaderboard(){
  ...
  multiEl("leaderboardList").innerHTML = entries.map((entry,index) => `
    <div class="leaderboard-row ${index < 3 ? "top-three" : ""}">
      ...
      <div class="leaderboard-record finish-rank">
        ${postseasonLabel(entry)}
        <small>${entry.champion ? "CHAMPION" : "POSTSEASON"}</small>
      </div>
    </div>
  `).join("");
}
...
window.loadDeveloperDashboard=async function(){
 ...
 try{
   ...
   const best=entries.sort(leaderboardSort)[0];
   ...
   multiEl("devBestRecord").textContent=best?postseasonLabel(best):"—";
 }catch(error){
