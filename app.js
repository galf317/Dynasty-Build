...
const ROOKIES_2026={"Las Vegas Raiders":{QB:[["Fernando Mendoza",82,"2026 Rookie • Round 1 Pick 1"]]},"Los Angeles Rams":{QB:[["Ty Simpson",78,"2026 Rookie • Round 1 Pick 13"]]},"Arizona Cardinals":{QB:[["Carson Beck",73,"2026 Rookie • Round 3 Pick 1"]],WR:[["Reggie Virgil",69,"2026 Rookie • Round 5 Pick 3"]]},"Kansas City Chiefs":{QB:[["Garrett Nussmeier",70,"2026 Rookie • Round 7 Pick 33"]],RB:[["Emmett Johnson",71,"2026 Rookie • Round 5 Pick 21"]]},"New Orleans Saints":{WR:[["Jordyn Tyson",80,"2026 Rookie • Round 1 Pick 8"],["Barion Brown",69,"2026 Rookie • Round 6 Pick 9"]]},"Cleveland Browns":{WR:[["KC Concepcion",79,"2026 Rookie • Round 1 Pick 24"],["Denzel Boston",77,"2026 Rookie • Round 2 Pick 7"]]},"New York Jets":{WR:[["Omar Cooper Jr.",78,"2026 Rookie • Round 1 Pick 30"]]},"Carolina Panthers":{WR:[["Chris Brazzell II",73,"2026 Rookie • Round 3 Pick 19"]]},"Atlanta Falcons":{WR:[["Zachariah Branch",74,"2026 Rookie • Round 3 Pick 15"]]},"San Francisco 49ers":{WR:[["De'Zhaun Stribling",75,"2026 Rookie • Round 2 Pick 1"]]},"Seattle Seahawks":{RB:[["Jadarian Price",79,"2026 Rookie • Round 1 Pick 32"]]},"Baltimore Ravens":{RB:[["Adam Randall",71,"2026 Rookie • Round 5 Pick 34"]]},"Minnesota Vikings":{RB:[["Demond Claiborne",69,"2026 Rookie • Round 6 Pick 17"]]}};
ROSTERS.forEach(t=>{const r=ROOKIES_2026[t[0]];if(r)["QB","RB","WR"].forEach(p=>{if(r[p])t[2][p].push(...r[p])})});
function applyLivePlayerData(){const live=window.LIVE_NFL_PLAYERS||[];const by={};ROSTERS.forEach(t=>by[t[0]]=t);live.forEach(p=>{const t=by[p.teamName];if(!t||!t[2][p.position])return;const e=t[2][p.position].find(x=>x[0].toLowerCase()===p.name.toLowerCase());const label=[p.depth||"Roster",p.injuryStatus?`Injury: ${p.injuryStatus}`:"",p.recentlyTraded?"Recently Traded":""].filter(Boolean).join(" • ");if(e)e[2]=label||e[2];else t[2][p.position].push([p.name,p.rating||70,label||"Live Roster"])});const s=document.getElementById("liveDataStatus");if(s)s.textContent=live.length?`Updated ${window.LIVE_NFL_UPDATED_AT||"recently"} • ${live.length} players`:"Built-in rosters • Live update unavailable"}setTimeout(applyLivePlayerData,0);
...
function show(id){["start","draw","built","draft","season","playoffs","final","lobby","multiplayerResults","leaderboard","developer","collection","achievements"].forEach(x=>$(x).classList.toggle("hidden",x!==id));scrollTo({top:0,behavior:"smooth"})}
...
function renderPlayerStrip(sequence,selectedIndex,traitName){
 $("playerStrip").innerHTML=sequence.map((p,i)=>{
   const label=p[2].toLowerCase();const isLegend=label.includes("legend"),isRookie=label.includes("2026 rookie"),isInjured=label.includes("injury:"),isTraded=label.includes("recently traded");
   return `<div class="player-tile ${isLegend?"legend":""}" data-index="${i}">
     ${isLegend?'<div class="legend-tag">LEGEND</div>':""}${isRookie?'<div class="rookie-tag">2026 ROOKIE</div>':""}${isInjured?'<div class="injury-tag">INJURY</div>':""}${isTraded?'<div class="trade-tag">TRADED</div>':""}
...
function finish(champ,text){
 ...
 evalAch(champ);setTimeout(()=>renderCard(champ),0);
...
const DAILY_CHALLENGES=[{title:"Pocket General",position:"QB",rule:"Build a QB and chase a championship."},{title:"Ground Control",position:"RB",rule:"Build an RB and finish with 12+ wins."},{title:"Route Technician",position:"WR",rule:"Build a WR with elite Hands and Routes."},{title:"No Safety Net",position:null,rule:"Complete any build without a respin."},{title:"Legend Hunt",position:null,rule:"Use at least one legend."}];const ACH=[{id:"first",icon:"🏈",name:"First Snap",desc:"Complete a career."},{id:"champ",icon:"🏆",name:"World Champion",desc:"Win the Super Bowl."},{id:"goat",icon:"👑",name:"GOAT Build",desc:"Create a 95+ OVR."},{id:"legend",icon:"⭐",name:"Legendary DNA",desc:"Use 3 legends."},{id:"perfect",icon:"💯",name:"Perfect Season",desc:"Finish 17-0."},{id:"collector",icon:"🎴",name:"Collector",desc:"Save 5 cards."}];function ch(){return DAILY_CHALLENGES[Math.floor(Date.now()/86400000)%DAILY_CHALLENGES.length]}function renderDaily(){const c=ch();$("dailyChallengeBanner").classList.remove("hidden");$("dailyChallengeTitle").textContent=c.title;$("dailyChallengeRule").textContent=c.rule;$("dailyChallengeCountdown").textContent=`${Math.floor((new Date().setHours(24,0,0,0)-Date.now())/3600000)}H LEFT`}renderDaily();function rarity(v){return v>=97?"GOAT":v>=93?"HALL OF FAME":v>=89?"ALL-PRO":v>=85?"PRO BOWL":v>=80?"STARTER":v>=73?"BACKUP":"PRACTICE SQUAD"}function cards(){try{return JSON.parse(localStorage.getItem("fplCards")||"[]")}catch(e){return[]}}function saveCards(v){localStorage.setItem("fplCards",JSON.stringify(v.slice(0,50)))}function aids(){try{return JSON.parse(localStorage.getItem("fplAchievements")||"[]")}catch(e){return[]}}function unlock(id){const a=aids();if(!a.includes(id)){a.push(id);localStorage.setItem("fplAchievements",JSON.stringify(a))}}function evalAch(c){unlock("first");if(c)unlock("champ");if(overall()>=95)unlock("goat");if(picks.filter(p=>String(p.depth).toLowerCase().includes("legend")).length>=3)unlock("legend");if(wins===17)unlock("perfect");if(cards().length>=5)unlock("collector")}function renderCard(c){const v=overall();$("cardRarity").textContent=rarity(v);$("cardOverall").textContent=v;$("cardPlayerName").textContent=name;$("cardMeta").textContent=`${pos} • ${team?team[0]:"Undrafted"}`;$("cardTeamLogo").innerHTML=team?teamLogoHtml(team[0],"large"):"";$("cardTraits").innerHTML=picks.map(p=>`<div class="card-trait"><span>${p.trait}</span><b>${p.rating}</b></div>`).join("");$("cardResult").textContent=c?`🏆 CHAMPION • ${wins}-${losses}`:`${postseasonLabel({champion:c,playoffStage:finalPlayoffStage})} • ${wins}-${losses}`;$("saveCareerCard").dataset.champion=c?"1":"0"}function saveCard(){const v={id:Date.now(),name,position:pos,overall:overall(),team:team?team[0]:"",wins,losses,champion:$("saveCareerCard").dataset.champion==="1",playoffStage:finalPlayoffStage,rarity:rarity(overall()),traits:picks};const c=cards();c.unshift(v);saveCards(c);evalAch(v.champion);$("saveCareerCard").textContent="Saved ✓"}function renderCollection(){const c=cards();$("collectionCount").textContent=`${c.length} CARDS`;$("collectionGrid").innerHTML=c.length?c.map(x=>`<div class="saved-card"><div class="saved-card-top"><span>${x.rarity}</span><b>${x.overall}</b></div><div>${x.team?teamLogoHtml(x.team):""}</div><h3>${x.name}</h3><small>${x.position} • ${x.team}</small><p>${x.champion?"🏆 CHAMPION":postseasonLabel(x)} • ${x.wins}-${x.losses}</p></div>`).join(""):"<div class='demo-log'>No saved cards yet.</div>"}function renderAchievements(){const a=aids();$("achievementCount").textContent=`${a.length} / ${ACH.length}`;$("achievementGrid").innerHTML=ACH.map(x=>`<div class="achievement ${a.includes(x.id)?"":"locked"}"><div class="achievement-icon">${x.icon}</div><div><strong>${x.name}</strong><small>${x.desc}</small></div><div>${a.includes(x.id)?"UNLOCKED":"LOCKED"}</div></div>`).join("")}$("modeDaily").onclick=()=>{const c=ch();if(c.position){pos=c.position;document.querySelectorAll(".pos").forEach(x=>x.classList.toggle("selected",x.dataset.pos===pos))}$("name").scrollIntoView({behavior:"smooth"})};$("modeCollection").onclick=()=>{show("collection");renderCollection()};$("modeAchievements").onclick=()=>{show("achievements");renderAchievements()};$("collectionHome").onclick=()=>show("start");$("achievementHome").onclick=()=>show("start");$("clearCollection").onclick=()=>{if(confirm("Delete saved cards?")){saveCards([]);renderCollection()}};$("saveCareerCard").onclick=saveCard;

(function initGridxPhaseOne(){
 const splash=document.getElementById("gridxSplash"),enter=document.getElementById("enterGridx"),particles=document.getElementById("splashParticles");
 if(particles){for(let i=0;i<28;i++){const p=document.createElement("span");p.className="splash-particle";p.style.left=`${Math.random()*100}%`;p.style.animationDuration=`${5+Math.random()*8}s`;p.style.animationDelay=`${Math.random()*-10}s`;p.style.opacity=`${.2+Math.random()*.7}`;particles.appendChild(p)}}
 function dismiss(){if(!splash||splash.classList.contains("dismissed"))return;splash.classList.add("dismissed");sessionStorage.setItem("gridxEntered","1");setTimeout(()=>splash.remove(),900)}
 if(sessionStorage.getItem("gridxEntered")==="1"&&splash){splash.classList.add("dismissed");setTimeout(()=>splash.remove(),100)}
 if(enter)enter.onclick=e=>{e.stopPropagation();dismiss()};if(splash){splash.onclick=dismiss;splash.onkeydown=e=>{if(e.key==="Enter"||e.key===" ")dismiss()}}
 const live=document.getElementById("liveDataStatus"),home=document.getElementById("homeRosterStatus");if(live&&home){const o=new MutationObserver(()=>home.textContent=live.textContent.toLowerCase().includes("unavailable")?"OFFLINE":"CONNECTED");o.observe(live,{childList:true,subtree:true,characterData:true})}
})();
