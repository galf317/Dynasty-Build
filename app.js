...
function show(id){["start","draw","built","draft","season","playoffs","final","lobby","multiplayerResults"].forEach(x=>$(x).classList.toggle("hidden",x!==id));scrollTo({top:0,behavior:"smooth"})}
...
$("begin").onclick=()=>{demoMode=false;name=$("name").value.trim()||"Rookie One";step=0;picks=[];respins=2;usedTraits=[];show("draw");renderTrait()};
...
function finish(champ,text){if(window.MULTIPLAYER_ACTIVE){window.submitMultiplayerResult(champ);return;}show("final");$("trophy").textContent=champ?"🏆":"🏈";$("finalTitle").textContent=champ?"Champions!":"Season Complete";$("finalText").textContent=text;const labels=pos==="QB"?[["PASS YDS",stats.yards],["PASS TD",stats.td],["INT",stats.to]]:pos==="RB"?[["RUSH YDS",stats.yards],["TOTAL TD",stats.td],["REC",stats.rec]]:[["REC YDS",stats.yards],["REC TD",stats.td],["REC",stats.rec]];$("finalStats").innerHTML=labels.map(x=>`<div class="stat"><b>${x[1]}</b>${x[0]}</div>`).join("")}
